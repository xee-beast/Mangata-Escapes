<template>
<div>
    <button @click="show = true" class="button is-medium is-fat is-rounded is-outlined is-black">UPDATE CARD ON FILE</button>
    <modal :is-active="show" @hide="hideModal">
        <div class="form-container">
            <div class="form-header">
                <div class="form-title">{{ group.name }} - Update Card On File</div>
            ...
            <div class="form-content is-font-family-montserrat">
                <lost-code v-if="lostCode" @sent="lostCode = false" :group="group.id" return-text="Update Payment Method" />
                ...
                <div>
                    <p class="heading">Current Card On File</p>
                </div>
                ...
                <travel-insurance-form v-model="card.insurance" :errors="cardErrors" :client="savedClient.name" :cancellationDate="group.cancellationDate" :groupsEmail="group.groupsEmail"/>
                </template>
                ...
                <terms-conditions-form v-model="card.confirmation" :errors="cardErrors" :url="group.termsConditionsUrl" :client="savedClient.name" :group="group" />
                </template>

                <template v-if="step > 2" class="is-font-family-montserrat">
                <p class="is-size-5 font-weight-600">Your billing information has been updated successfully.</p>
                <br>
...
export default {
    ...
    methods: {
        ...
        next() {
            this.cardErrors = {};
            this.isLoading = true;
            let request = this.$http.post(`/groups/${this.group.id}/update-card/${this.step}`, this.card)
                .then(response => {

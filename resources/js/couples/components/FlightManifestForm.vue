<template>
<div>
    <button @click="show = true" class="button is-medium is-fat is-rounded is-outlined is-black">FLIGHT ITINERARY</button>
    <modal :is-active="show" @hide="hideModal">
        <div class="form-container">
            <div class="form-header">
                <div class="form-title">{{ group.name }} - Flight Itinerary</div>
            ...
            <div class="form-content is-font-family-montserrat">
                <lost-code v-if="lostCode" @sent="lostCode = false" :group="group.id" return-text="Flight Itinerary"/>
                ...
                <template v-else>
                    ...
                    <template v-if="step == 2" class="is-font-family-montserrat">
                        <template v-if="transportation">
                            ...
                            <template  v-if="!clone">
                                <div class="form-seperator is-font-family-montserrat">
                                    <label class="label font-weight-600">Please select the guests to which the following flight information applies. If any guest is on a different flight, you can submit the form again.</label>
                                </div>
                                    ...
                                    <div class="columns" v-if="shouldShowArrivalFields">
                                        <div class="column">
                                            <form-field label="Arrival Airport" class="control" :errors="formErrors['form.arrivalAirport']">
                                                <control-select
                                                    ...
                                                    :options="[ { value: '', text: 'Select Airport', disabled: true }, ...group.airports ]"
                                                    default-value=""
                                        ...
                                        <div class="column">
                                            <form-field label="Arrival Airline" class="control" :errors="formErrors['form.arrivalAirline']">
                                                    ...
                                                    />
                                                    <span class="span-description">If your airline does not appear above, please forward your flight confirmation to {{ group.groupsEmail }} for further assistance.</span>
                                            </form-field>
                                        </div>
                                    </div>
                                    <div class="columns" v-if="shouldShowArrivalFields">
                        ...
                        <template v-else>
                            <p class="is-size-5 font-weight-600">Your booking does not include airport transfers.</p>
                            <br>
                            <p>If you wish to add transfers to your booking or you believe this is a mistake, please contact us at <a class="has-text-link" :href="`mailto:${group.groupsEmail}`">{{ group.groupsEmail }}</a>.</p>
                        </template>
                    </template>
                    <template v-if="step > 2" class="is-font-family-montserrat">
                        <p class="is-size-5 font-weight-600">
                            Thank you for submitting your flight itinerary! You will receive travel documents with instructions on how to find your shuttle when you arrive approximately 1 week prior to your departure. If you have any questions or concerns, please do not hesitate to contact us at {{ group.groupsEmail }} or by phone or text at 866-822-7336.
                        </p>
                    </template>
                </template>
            </div>
            <div v-if="!lostCode && step <= 2 && quoteAccepted" class="form-footer">
                <div class="columns is-mobile">
                    ...
                    <div v-if="transportation" class="column is-narrow">
                        <button v-if="step != 2 && !showErrorAlert && isGroupEventDate" @click="next" class="button is-dark is-outlined" :class="{ 'is-loading': isLoading }">Next</button>
                        <button v-else-if="flightManifest.guests.length > 0 && !showErrorAlert && isGroupEventDate" @click="next" class="button is-dark is-outlined" :class="{ 'is-loading': isLoading }">Submit Manifest</button>
    ...
    <!-- dates mismatched radio selections -->
    <modal :is-active="showModal" @hide="closeModal">
        <div class="form-container">
            <div class="form-header">
                <div class="form-title">{{ group.name }} - Flight Itinerary</div>
                <button type="button" class="modal-close-booking" aria-label="close" @click="closeModal">
                    <span class="icon"><i class="fas fa-times"></i></span>
                </button>
            </div>
...
export default {
    ...
    methods: {
        ...
        close() {
            if (this.lostCode || this.showErrorAlert || this.step > 2 || (this.step === 2 && (!this.transportation || this.flightManifest.guests.length === 0))) {
        ...
        next() {
            ...
            if (this.submitManifest) {
                ...
                let request = this.$http.post(`/groups/${this.group.id}/new-flight-manifest/${this.step}`, this.flightManifest)
                    .then(response => {

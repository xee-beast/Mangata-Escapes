<template>
	<card title="Payments" :booking-status="booking.deletedAt ? 'cancelled' : 'active'">
		<template v-slot:action>
			<create-payment v-if="can.create" :booking="booking" :countries="countries" @created="fetchData()" button-class="is-outlined is-primary is-inverted" />
		</template>
		<template v-if="payments">
			<data-table class="is-size-6" table-class="is-fullwidth" :columns="['Amount', 'Status', 'Client', 'Payment', 'Notes', 'Actions']">
				<template v-if="payments.length">
					<tr v-for="payment in payments" :key="payment.id">
						...
						<td>{{ payment.bookingClient.firstName }} {{ payment.bookingClient.lastName }}</td>
						<td>
			...
			<show-payment v-if="showPayment" :payment="showPayment" :show-sensitive="can.processPayments" @updated="updated" @canceled="showPayment = null" />
...
<script>
	...
	import ConfirmPayment from '@dashboard/pages/IndividualBookings/Payments/Confirm';
	import CreatePayment from '@dashboard/pages/IndividualBookings/Payments/Create';
	import DataTable from '@dashboard/components/table/Table';
	import DeletePayment from '@dashboard/pages/IndividualBookings/Payments/Delete';
	import ForceDeletePayment from '@dashboard/pages/IndividualBookings/Payments/ForceDelete';
	import ShowPayment from '@dashboard/pages/IndividualBookings/Payments/Show';
	...
	export default {
		...
		methods: {
			fetchData() {
				this.$http.get('/individual-bookings/' + this.$route.params.id + '/payments')
					.then(response => {
			...
			setBreadcrumbs() {
				this.$store.commit('breadcrumbs', [
					...
					{
						label: 'Payments',
						route: 'individual-bookings.payments',
						params: {
							id: this.booking.id
						}
					}

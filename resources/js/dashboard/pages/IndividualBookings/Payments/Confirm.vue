<template>
	...
	<p>Are you sure you want to confirm a payment of ${{ payment.amount }} for {{ payment.bookingClient.firstName }} {{ payment.bookingClient.lastName }}? <br><br></p>
	...
	export default {
		...
		methods: {
			confirm() {
				...
				let request = this.$http.patch(`/individual-bookings/${this.$route.params.id}/payments/${this.payment.id}/confirm`, { 'sendEmail': this.sendEmail })
					.then(response => {
						this.$emit('confirmed', response.data.data);

						this.$store.commit('notification', {
							type: 'success',
							message: `A payment of ${this.payment.amount} has been confirmed for ${this.payment.bookingClient.firstName} ${this.payment.bookingClient.lastName}`
						});
					});

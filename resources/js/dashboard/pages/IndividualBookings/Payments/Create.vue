					...
					<template v-if="!payment.useExistingCard">
						<form-panel label="Credit Card Information">
							<form-field label="Cardholder Name" :errors="paymentErrors['card.name']" :required="true">
								<control-input v-model="payment.card.name" class="is-capitalized" :class="{ 'is-danger': (paymentErrors['card.name'] || []).length }" />
							</form-field>
							<form-field label="Credit Card Number" :errors="[...(paymentErrors['card.number'] || []), ...(paymentErrors['card.type'] || [])]" :required="true">
								<control-input v-model="payment.card.number" @input="setCardType" :class="{ 'is-danger': (paymentErrors['card.number'] || []).length }" />
							...
							<div class="columns">
								<div class="column is-4">
									<form-field label="Expiration Date" :errors="[...(paymentErrors['card.expMonth'] || []), ...(paymentErrors['card.expYear'] || [])]" :required="true">
										<control-select v-model="payment.card.expMonth" :options="expMonths" first-is-empty :class="{ 'is-danger': (paymentErrors['card.expMonth'] || []).length }" />
										<template v-slot:addon>
											<control-select v-model="payment.card.expYear" :options="expYears" first-is-empty :class="{ 'is-danger': (paymentErrors['card.expYear'] || []).length }" />
										</template>
									</form-field>
								</div>
								<div class="column is-4">
						...
						<form-panel label="Billing Address">
							<form-field label="Country" :errors="paymentErrors['address.country']" :required="true">
								<control-select
									v-model="payment.address.country"
							...
							<form-field v-if="!payment.address.country" :errors="paymentErrors['address.otherCountry']">
								<control-input v-model="payment.address.otherCountry" :class="{ 'is-danger': (paymentErrors['address.otherCountry'] || []).length }" />
							</form-field>
							<div class="columns">
								<div class="column">
									<form-field v-if="payment.address.country" label="State/Province" :errors="paymentErrors['address.state']" :required="true">
										<control-select
											v-model="payment.address.state"
											:options="selectedCountry ? selectedCountry.states.map(state => ({value: state.id, text: state.name})) : []"
											first-is-empty
											:class="{ 'is-danger': (paymentErrors['address.state'] || []).length }"
										/>
									</form-field>
									<form-field v-else label="State/Province" :errors="paymentErrors['address.otherState']" :required="true">
										<control-input v-model="payment.address.otherState" :class="{ 'is-danger': (paymentErrors['address.otherState'] || []).length }" />
									</form-field>
								</div>
								<div class="column">
									<form-field label="City" :errors="paymentErrors['address.city']" :required="true">
										<control-input v-model="payment.address.city" :class="{ 'is-danger': (paymentErrors['address.city'] || []).length }" />
									</form-field>
								</div>
							</div>
							...
							<div class="columns">
								<div class="column is-8">
									<form-field label="Address Line 2" :errors="paymentErrors['address.line2']">
										<control-input v-model="payment.address.line2" :class="{ 'is-danger': (paymentErrors['address.line2'] || []).length }" />
									</form-field>
								</div>
								<div class="column is-4">
									<form-field label="Zip/Postal Code" :errors="paymentErrors['address.zipCode']" :required="true">
										<control-input v-model="payment.address.zipCode" :class="{ 'is-danger': (paymentErrors['address.zipCode'] || []).length }" />
									</form-field>
								</div>
							</div>
						</form-panel>
	...
	export default {
		...
		methods: {
			create() {
				...
				let request = this.$http.post(`/individual-bookings/${ this.$route.params.id }/payments`, this.payment)
					.then(response => {
						this.close();

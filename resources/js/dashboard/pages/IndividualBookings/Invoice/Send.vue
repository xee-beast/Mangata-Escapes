<template>
    <div>
        <modal @hide="close" title="Send Invoice" :is-active="true">
            <p>
                {{ this.booking.clients.length > 1 ? 'Please select the clients to whom you would like to send the invoice.' : 'Are you sure you want to send the invoice to ' + this.clientText + '?' }}
            </p>
            <br>
    ...
    export default {
        ...
        methods: {
            confirm() {
                ...
                let request = this.$http.post('/individual-bookings/' + this.booking.id + '/send-invoice', { 'clients': this.selectedClients.map(client => client.id) })
                    .then(() => {
                        this.$emit('invoiceSent', this.booking);

				...
				<div class="column">
					<form-field label="First Name" :errors="clientErrors['firstName']" :required="true">
						<control-input v-model="client.firstName" class="is-capitalized"
							:class="{ 'is-danger': (clientErrors['firstName'] || []).length }" />
					</form-field>
				</div>
				<div class="column">
					<form-field label="Last Name" :errors="clientErrors['lastName']" :required="true">
						<control-input v-model="client.lastName" class="is-capitalized"
							:class="{ 'is-danger': (clientErrors['lastName'] || []).length }" />
					</form-field>
				</div>
			</div>
			...
			<form-panel label="Billing Information" class="is-borderless">
				<form-panel label="Credit Card Information">
					<form-field label="Cardholder Name" :errors="clientErrors['card.name']" :required="true">
						<control-input v-model="client.card.name" class="is-capitalized"
							:class="{ 'is-danger': (clientErrors['card.name'] || []).length }" />
					</form-field>
					<form-field label="Credit Card Number" :errors="[...(clientErrors['card.number'] || []), ...(clientErrors['card.type'] || [])]" :required="true">
						<control-input v-model="client.card.number" @input="setCardType"
							:class="{ 'is-danger': (clientErrors['card.number'] || []).length }" />
					...
					<div class="columns">
						<div class="column is-4">
							<form-field label="Expiration Date"
								:errors="[...(clientErrors['card.expMonth'] || []), ...(clientErrors['card.expYear'] || [])]" :required="true">
								<control-select v-model="client.card.expMonth" :options="expMonths" first-is-empty
									:class="{ 'is-danger': (clientErrors['card.expMonth'] || []).length }" />
								<template v-slot:addon>
									<control-select v-model="client.card.expYear" :options="expYears" first-is-empty
										:class="{ 'is-danger': (clientErrors['card.expYear'] || []).length }" />
								</template>
							</form-field>
						</div>
						<div class="column is-4">
				...
				<form-panel label="Billing Address">
					<form-field label="Country" :errors="clientErrors['address.country']" :required="true">
						<control-select v-model="client.address.country"
							:options="[...countries.map(country => ({value: country.id, text: country.name})), {value: 0, text: 'Other...'}]"
							:class="{ 'is-danger': (clientErrors['address.country'] || []).length }" :disabled="client.address.hasOtherCountry" />
					</form-field>
					<form-field v-if="client.address.country == 0" :errors="clientErrors['address.otherCountry']">
						<control-input v-model="client.address.otherCountry"
							:class="{ 'is-danger': (clientErrors['address.otherCountry'] || []).length }" />
					</form-field>
					<div class="columns">
						<div class="column">
							<form-field v-if="client.address.country != 0" label="State/Province" :errors="clientErrors['address.state']" :required="true">
								<control-select v-model="client.address.state"
									:options="selectedCountry ? selectedCountry.states.map(state => ({value: state.id, text: state.name})) : []"
									first-is-empty :class="{ 'is-danger': (clientErrors['address.state'] || []).length }" />
							</form-field>
							<form-field v-else label="State/Province" :errors="clientErrors['address.otherState']" :required="true">
								<control-input v-model="client.address.otherState"
									:class="{ 'is-danger': (clientErrors['address.otherState'] || []).length }" />
							</form-field>
					...
					<div class="columns">
						...
						<div class="column is-4">
							<form-field label="Zip/Postal Code" :errors="clientErrors['address.zipCode']" :required="true">
								<control-input v-model="client.address.zipCode"
									:class="{ 'is-danger': (clientErrors['address.zipCode'] || []).length }" />
							</form-field>
						</div>
					</div>
				</form-panel>
...
export default {
	...
	methods: {
		create() {
			...
			let request = this.$http.post(`groups/${this.$route.params['group']}/bookings/${this.$route.params['booking']}/clients`, this.client)
				.then(response => {
					this.close();

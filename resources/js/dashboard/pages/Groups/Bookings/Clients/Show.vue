...
<card v-if="savedClient" :title="`${savedClient.firstName} ${this.savedClient.lastName}`" :booking-status="booking.deletedAt ? 'cancelled' : 'active'">
	<template v-slot:action v-if="savedClient.can.delete || savedClient.can.viewPayments">
		<router-link v-if="savedClient.can.viewPayments" :to="{ name: 'payments', params: { group: group.id, booking: booking.id }}" class="button is-outlined is-primary is-inverted">
	...
	<template v-slot:tabs>
		<tabs class="is-boxed">
			...
			<tab @click="setTab('extras')" :is-active="tabs.extras">{{ group.fit ? 'Rates' : 'Extras' }}</tab>
		</tabs>
	</template>
		...
		<div class="columns">
			<div class="column">
				<form-field label="First Name" :errors="clientErrors['firstName']" :required="true">
					<control-input v-model="client.firstName" class="is-capitalized"
						:class="{ 'is-danger': (clientErrors['firstName'] || []).length }" :readonly="readonly" />
				</form-field>
			</div>
			<div class="column">
				<form-field label="Last Name" :errors="clientErrors['lastName']" :required="true">
					<control-input v-model="client.lastName" class="is-capitalized"
						:class="{ 'is-danger': (clientErrors['lastName'] || []).length }" :readonly="readonly" />
				</form-field>
			</div>
		</div>
		<div class="columns">
	...
	<template v-if="tabs.card">
		<form-panel label="Credit Card Information">
			<form-field label="Cardholder Name" :errors="cardErrors['card.name']" :required="true">
				<control-input v-model="card.card.name" class="is-capitalized"
					:class="{ 'is-danger': (cardErrors['card.name'] || []).length }" />
			</form-field>
			<form-field label="Credit Card Number" :errors="[...(cardErrors['card.number'] || []), ...(cardErrors['card.type'] || [])]" :required="true">
				<control-input v-model="card.card.number" @input="setCardType"
					:class="{ 'is-danger': (cardErrors['card.number'] || []).length }" />
			...
			<div class="columns">
				<div class="column is-4">
					<form-field label="Expiration Date"
						:errors="[...(cardErrors['card.expMonth'] || []), ...(cardErrors['card.expYear'] || [])]" :required="true">
						<control-select v-model="card.card.expMonth" :options="expMonths" first-is-empty
							:class="{ 'is-danger': (cardErrors['card.expMonth'] || []).length }" />
						<template v-slot:addon>
							<control-select v-model="card.card.expYear" :options="expYears" first-is-empty
								:class="{ 'is-danger': (cardErrors['card.expYear'] || []).length }" />
						</template>
					</form-field>
				</div>
				<div class="column is-4">
			...
			<div class="columns">
				...
				<div class="column is-4">
					<form-field label="Zip/Postal Code" :errors="cardErrors['address.zipCode']" :required="true">
						<control-input v-model="card.address.zipCode"
							:class="{ 'is-danger': (cardErrors['address.zipCode'] || []).length }" />
					</form-field>
				</div>
			</div>
		</form-panel>
	...
	<template v-if="tabs.extras">
		<div class="columns" v-if="group.fit">
			<div class="column">
				<form-field label="Accommodation Charges" :errors="extrasErrors['fitRate.accommodation']" :required="true">
		...
		<h2 v-if="group.fit"><b>Extras</b></h2>
...
export default {
	...
	methods: {
		fetchData() {
			this.$http.get(`groups/${this.$route.params['group']}/bookings/${this.$route.params['booking']}/clients/${this.$route.params['id']}`)
				.then(response => {
		...
		update() {
			...
			let request = this.$http.put(`groups/${this.$route.params['group']}/bookings/${this.$route.params['booking']}/clients/${this.$route.params['id']}`, this.client)
				.then(response => {
		...
		updateCard() {
			...
			let request = this.$http.patch(`groups/${this.$route.params['group']}/bookings/${this.$route.params['booking']}/clients/${this.$route.params['id']}/card`, this.card)
				.then(response => {
					this.$store.commit('notification', {
						type: 'success',
		...
		syncExtras() {
			...
			let request = this.$http.patch(`groups/${this.$route.params['group']}/bookings/${this.$route.params['booking']}/clients/${this.$route.params['id']}/extras`, { extras: this.extras, fitRate: this.fitRate })
				.then(response => {
					this.$store.commit('notification', {
						type: 'success',

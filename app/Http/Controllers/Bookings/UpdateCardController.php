...
use App\Http\Requests\Bookings\UpdateCard;
use Carbon\Carbon;
use Illuminate\Validation\ValidationException;

class UpdateCardController extends Controller
{
    public function updateCard($step, UpdateCard $request) 
    {
        ...
        if ($clientBooking->booking->balance_due_date && Carbon::today()->greaterThan($clientBooking->booking->balance_due_date)) {
            throw ValidationException::withMessages([
                'booking.code' => ['The balance due date has passed. You can no longer update your card from here. Please contact us at ' . config('emails.groups') . ' if you need any assistance.']
            ]);
        }
        ...
        if ($step < 2) {
            return response()->json([
                'booking' => $clientBooking->booking->load(['bookingDueDates']),
                'client' => [

...
class FlightManifestDepartureSheet implements FromCollection, WithHeadings, ShouldAutoSize, WithTitle, WithStyles
{
    private $group;
    ...
    public function collection()
    {
        ...
        $bookings = $this->group->bookings()
            ->with([
                'clients' => function ($query) {
                    $query->with([
                        'guests.flight_manifest.departureAirport',
                    ])->whereHas('guests');
                },
            ])
            ->ordered()
        ...
        foreach ($bookings as $booking) {
            foreach ($booking->clients as $client) {
                foreach ($client->guests as $guest) {
                     if ($guest->transportation && in_array($guest->transportation_type, [ Booking::TRANSPORTATION_TYPE_ROUND_TRIP, Booking::TRANSPORTATION_TYPE_ONE_WAY_HOTEL_TO_AIRPORT])){
                        if (!is_null($guest->flight_manifest)) {
                            $flight_manifests->push([
                                ...
                                'departure_airport' => isset($guest->flight_manifest->departureAirport) ? $guest->flight_manifest->departureAirport->airport_code : null,
                                'departure_flight' => isset($guest->flight_manifest->departure_airline) ?  $guest->flight_manifest->getAirlineName($guest->flight_manifest->departure_airline) . ' ' . $guest->flight_manifest->departure_number : null,
                                'departure_date' => isset($guest->flight_manifest->departure_datetime) ? Carbon::parse($guest->flight_manifest->departure_datetime, 'UTC')->setTimezone(isset($guest->flight_manifest->departureAirport) ? $guest->flight_manifest->departureAirport->timezone : 'UTC') : null,
                            ]);
                        } else {
                            $flight_manifests->push([
                                'booking_order' => $booking->order,
        ...
        $sorted_flight_manifests = $flight_manifests->sortBy(function ($manifest) {
            return [
                $manifest['departure_date'] ? strtotime($manifest['departure_date']) : PHP_INT_MAX,
        ...
        foreach ($sorted_flight_manifests as $manifest) {
            ...
            $manifest['departure_time'] = $manifest['departure_date'] ? $manifest['departure_date']->format('H:i') : null;
            $manifest['departure_date'] = $manifest['departure_date'] ? $manifest['departure_date']->format('M d, Y') : null;

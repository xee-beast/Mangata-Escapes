...
class FinalFlightManifestReminder extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if (count($guestNames) === 1) {
            ...
            $guestLine2 = "Airport transfers are not scheduled for this guest.";
        } elseif (count($guestNames) === 2) {
            $guestLine = "We have not received a flight itinerary for {$guestNames[0]} and {$guestNames[1]}.";
            $guestLine2 = "Airport transfers are not scheduled for these guests.";
        } else {
        ...
        return (new MailMessage)
            ->subject("{$subject} {$this->bookingClient->reservation_code} - Flight Itinerary Submission Date Passed")
            ->greeting('Hello…')
            ->line($guestLine)
            ->line($guestLine2)
            ->line('Upload their flight itinerary here using your reservation code <b>' . $this->bookingClient->reservation_code . '</b> by ' . Carbon::today()->addDays(5)->format('F jS, Y') . ' or a shuttle will not be scheduled.')
            ->action($action, $route)
